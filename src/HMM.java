...
public class HMM {
	public enum DriverAttentionState {
		...
		private enum StartProbabilities {
	        ...
	        StartProbabilities() {
	            data = new HashMap<>();
	            data.put(DriverState.ATTENTIVE, 0.8);
	            data.put(DriverState.INATTENTIVE, 0.2);
	        }
	    }
		...
		private enum TransitionProbabilities {
	        ...
	        TransitionProbabilities() {
	            data = new HashMap<>();
	            data.put(new Transition<>(DriverState.ATTENTIVE, DriverState.ATTENTIVE), 0.9);
	            data.put(new Transition<>(DriverState.ATTENTIVE, DriverState.INATTENTIVE), 0.1);
